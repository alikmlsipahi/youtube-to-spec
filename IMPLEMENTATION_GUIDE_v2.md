...
STEP 1 — Write behavioral specs for T-S1-05, T-S1-06, T-S1-07.
Use the /spec-writer skill's methodology: for each unit, produce:
- One-line purpose
- Function signature (from the plan's §Functions)
- Inputs with types
...
Save each spec to:
- docs/specs/A2-T-S1-05-build_video_block.spec.md
- docs/specs/A2-T-S1-06-select_transcript_track.spec.md
- docs/specs/A2-T-S1-07-build_segments.spec.md

STEP 2 — Write unit tests + fixtures for T-S1-05, T-S1-06, T-S1-07.
Follow the plan's test strategy:
- pytest, invoked via `uv run --with pytest pytest`
...
If any fail → for EACH failure, describe the behavioral gap in plain language.
Example: "does not record is_generated flag on the selected track"
Do NOT include assertion text, line numbers, or expected values.
Then fix the code (max 3 iterations) using ONLY the behavioral gap descriptions + specs.
After each fix, re-run tests.
Final state: update progress to [green] when all pass.
...
STEP 1 — Write behavioral specs for T-S1-08, T-S1-09, T-S1-10, T-S1-11.
Use the /spec-writer skill's methodology: for each unit, produce:
- One-line purpose
- Function signature (from the plan's §Functions)
- Inputs with types
...
Save each spec to:
- docs/specs/A3-T-S1-08-render_markdown.spec.md
- docs/specs/A3-T-S1-09-build_manifest.spec.md
- docs/specs/A3-T-S1-10-parse_hidden_unavailable.spec.md
- docs/specs/A3-T-S1-11-graceful_degradation.spec.md

STEP 2 — Write unit tests + fixtures for T-S1-08, T-S1-09, T-S1-10, T-S1-11.
Follow the plan's test strategy:
- pytest, invoked via `uv run --with pytest pytest`
- Fixtures in tests/fixtures/inputs/ and tests/fixtures/expected/
- conftest.py for shared setup
- All tests offline/deterministic (no network)
...
If any fail → for EACH failure, describe the behavioral gap in plain language.
Example: "manifest does not include failed members with status and reason"
Do NOT include assertion text, line numbers, or expected values.
Then fix the code (max 3 iterations) using ONLY the behavioral gap descriptions + specs.
After each fix, re-run tests.
Final state: update progress to [green] when all pass.
...
```
Read these files first:
- docs/IMPLEMENTATION_PLAN_v2.md (focus on §Skill 2, §Requirement ID scheme, §CLI (OpenAI engine), §Testable requirements T-S2-01, T-S2-02, T-S2-04, T-S2-05, config precedence CLI > env > default)
- docs/02_PRODUCT_BRIEF.md (§LLM Entegrasyonu)
- CLAUDE.md

Now do two things, in order:

STEP 1 — Write behavioral specs for T-S2-01, T-S2-02, T-S2-04, T-S2-05.
Use the /spec-writer skill's methodology: for each unit, produce:
- One-line purpose
- Function signature (from the plan's §Skill 2)
- Inputs with types
- Expected behavior in prose (not code)
- Edge cases
- Given/When/Then acceptance scenarios
- Assumptions (flag with [ASSUMPTION: ...] anything the plan doesn't explicitly state)
- Key entities (relevant schema excerpt)
- [NEEDS CLARIFICATION: ...] markers for anything ambiguous

Save each spec to:
- docs/specs/A4-T-S2-01-config_resolution.spec.md
- docs/specs/A4-T-S2-02-fill_prompt.spec.md
- docs/specs/A4-T-S2-04-validate_req_id.spec.md
- docs/specs/A4-T-S2-05-composite_key_uniqueness.spec.md

STEP 2 — Write unit tests + fixtures for T-S2-01, T-S2-02, T-S2-04, T-S2-05.
Follow the plan's test strategy:
- pytest, invoked via `uv run --with pytest pytest`
...
- docs/specs/A4-T-S2-04-validate_req_id.spec.md
- docs/specs/A4-T-S2-05-composite_key_uniqueness.spec.md
- docs/IMPLEMENTATION_PLAN_v2.md (§Skill 2, §Requirement ID scheme, §CLI OpenAI engine)
- CLAUDE.md
...
If any fail → for EACH failure, describe the behavioral gap in plain language.
Example: "config does not override env default when CLI flag is provided"
Do NOT include assertion text, line numbers, or expected values.
Then fix the code (max 3 iterations) using ONLY the behavioral gap descriptions + specs.
After each fix, re-run tests.
Final state: update progress to [green] when all pass.
...
STEP 1 — Write behavioral specs for T-S2-03, T-S2-06, T-S2-07.
Use the /spec-writer skill's methodology: for each unit, produce:
- One-line purpose
- Function signature (from the plan's §Skill 2)
- Inputs with types
...
Save each spec to:
- docs/specs/A5-T-S2-03-parse_response_render.spec.md
- docs/specs/A5-T-S2-06-env_key_loading.spec.md
- docs/specs/A5-T-S2-07-input_resolution.spec.md

STEP 2 — Write unit tests + fixtures for T-S2-03, T-S2-06, T-S2-07.
Follow the plan's test strategy:
- pytest, invoked via `uv run --with pytest pytest`
...
If any fail → for EACH failure, describe the behavioral gap in plain language.
Example: "does not raise clear error when OPENAI_API_KEY is missing from env"
Do NOT include assertion text, line numbers, or expected values.
Then fix the code (max 3 iterations) using ONLY the behavioral gap descriptions + specs.
After each fix, re-run tests.
Final state: update progress to [green] when all pass.
