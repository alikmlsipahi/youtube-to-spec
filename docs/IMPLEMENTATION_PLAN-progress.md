...
- [accepted] T-S1-16 — classify_failure — **[v2.3] the precondition for retry.** 52 cases green. String-based on `(name, text)`, not exception classes: the tier stubs the network deps in `sys.modules`, so an `except` on a stubbed attribute would take the whole tier down. **The blind implementer caught what the spec author missed** — YouTube ships the bot check with a typographic apostrophe (U+2019) while the spec's signal was ASCII, so the highest-consequence transient signal would have silently failed to match its own real text and classified `"permanent"`. Folded into the spec as contract and pinned after the fact. Guard proven: dropping the fold fails 5 of 52.
  - [accepted] **[v2.5]** third verdict `"unknown"`. The fall-through returned `"permanent"` too, so the unit computed the difference between *knowing* a failure is permanent and *recognizing nothing* — and threw it away. Same information-destroying collapse as `fetch_metadata`'s pre-v2.3 `None`, one level up. 52 → 56 cases; tier 388 → 394. Retry policy untouched (callers test `!= "transient"`); the value is that a caller can refuse to write a conclusion it never reached. Guard proven: reverting the fall-through fails 15 of 394.
    - **The canary caught a real hole on day one.** A private *playlist* matched no signal — the list says `private video` / `this video is private`, and a playlist says *playlist*. It had been answering `"permanent"` **by accident** via the fall-through, and the test covering it was green **for the wrong reason**. Signal added. This is the entry worth remembering: the collapse was not just a latent risk, it was already hiding a live gap.
    - **Live drift proof**, same captions-disabled video, `TranscriptsDisabled` renamed to simulate the library renaming its own exception: drifted → exit 1, **0 files**, the library's own *"Subtitles are disabled for this video"* printed verbatim; real → exit 0, 2 files, written as a complete artifact. The drifted run is precisely the scenario that lost a transcript in silence before v2.5.
    - **Accepted limit, recorded rather than hidden:** this catches drift on *first occurrence*, not before. Proactive detection was ruled out **on measurement**: a frozen fixture pins text already seen, and a scan of the installed yt-dlp finds `Private video` and `members-only` but **zero** occurrences of `not a bot`, `Too Many Requests` or `HTTP Error 429` — the signals that matter come from YouTube's servers, not yt-dlp's code.
...
- [done] SK1-ORCH — extract_artifacts.py main/CLI/IO + enumerate_playlist/fetch_transcript/build_artifact/write_* (verified by I-01 ✓, I-02 ✓)
  ...
  - [done] **[v2.5]** the drift canary. `main()`'s transcript branch handled only `"transient"` and let everything else be written as `available: false` — so an *unrecognized* failure was recorded as "this video has no captions". Now `"permanent"` means an **established** absence (written, complete) and `"unknown"` establishes nothing (**not** written, status `unrecognized`). This is what makes drift harmless **without touching the retry policy**. `_classify_and_report` prints the raw text at the six delegation sites — the only moment the evidence exists, since it was destroyed at the return statement and an ephemeral 429 cannot be reproduced later. `_warn_failed` gained a third message: telling a user not to re-run a possibly-drifted 429 would be wrong advice.
    - **The gate manufactures the drift rather than waiting for it.** It runs a *copy* of the script with one signal renamed — real classifier, real fetch, real `main()`, real YouTube; only the string table moved, which is a faithful model of the one thing that actually goes wrong. It **asserts the mutation applied**: a sabotage that silently missed would be an unmutated script passing every assertion — a dead gate reporting green. It pairs a control run on the same video so "wrote nothing" means something, and skips rather than passes on every confound.
    - **A fourth rule, earned the same way as the others.** v2.3: green units don't mean a working program. v2.4: a green gate doesn't either. v2.5: **a green test can be green for the wrong reason** — the private-playlist test passed for a year by riding a fall-through, not a match. The collapse that made it pass is the same one that would have hidden real drift.
